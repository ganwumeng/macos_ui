      ...
      child: Semantics(
        ...
        child: IntrinsicWidth(
          child: MacosOverlayFilter(
            color: MacosPulldownButtonTheme.of(context).pulldownColor?.withValues(alpha: 0.8),
            borderRadius: _kBorderRadius,
            child: Padding(
...
class MacosPulldownButton extends StatefulWidget {
  ...
  const MacosPulldownButton({
    ...
    this.menuAlignment = PulldownMenuAlignment.left,
        ...
        assert((title != null || icon != null) && !(title != null && icon != null), "There should be either a title or an icon argument provided, and not both at at the same time.");
...
class _MacosPulldownButtonState extends State<MacosPulldownButton> with WidgetsBindingObserver {
...
Future showPulldownMenu(
    ...
    for (int index = 0; index < items!.length; index += 1)
      _MenuItem(
        ...
        onLayout: (Size size) {
          ...
          // collect each item's height to lay out is fine since the route is
          // already on its way out.
          if (pulldownRoute == null) return;

          pulldownRoute.itemHeights[index] = size.height;
        },
      ),
  ];

  final NavigatorState navigator = Navigator.of(context);
  assert(pulldownRoute == null);
  final RenderBox itemBox = context.findRenderObject()! as RenderBox;
  itemRect ??= itemBox.localToGlobal(
      ...
      itemBox.size;
  pulldownRoute = _MacosPulldownRoute(
    items: menuItems,
    buttonRect: menuMargin.resolve(textDirection).inflateRect(itemRect),
    padding: _kMenuItemPadding.resolve(textDirection),
    capturedThemes: InheritedTheme.capture(from: context, to: navigator.context),
    style: textStyle ?? MacosTheme.of(context).typography.body,
    barrierLabel: MaterialLocalizations.of(context).modalBarrierDismissLabel,
    itemHeight: itemHeight,
    menuAlignment: menuAlignment,
  );

  return navigator.push(pulldownRoute);
}
